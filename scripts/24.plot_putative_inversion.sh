#!/bin/sh
#
# plot_inversions.sh — Generate multi-panel figures (MDS, local PCA, heterozygosity, LD)
#                      for each putative inversion.
#
# INPUT REQUIRED:
#
#   1. From 20.MDS matrix
#        - mds_matrix.txt       (chromosome, midpoint position, mds values)
#        - inversion_info.txt   (LGC ID, chromosome, MDS axis, thresholds)
#
#   2. From 21.local.pca output
#        - <LGC>.cov            (local covariance matrix)
#        - <LGC>_geno.txt       (cluster calls per inversion)
#        - pop_info_rm8.txt     (sample ID + location)
#
#   3. From 22.het output
#        - <LGC>.het            (heterozygosity per sample)
#
#   4. From 23.LD output
#        - <LGC>.ld             (LD for all individuals)
#        - <LGC>.AA.ld          (LD for homozygous cluster)
#
# OUTPUT:
#    For each LGC:
#      - <LGC>.png
...
################################################################################
# INPUT: MDS MATRIX + INVERSION INFO
################################################################################

mds  <- read.table("/local_path/pmax2023/out/13.MDS_pca_het_LD/mds_matrix.txt",
                   header = TRUE)
info <- read.table("/local_path/pmax2023/out/13.MDS_pca_het_LD/inversion_info.txt",
                   header = TRUE)
...
mds_plot <- function(mds_df, inv_row) {
  ...
  mds_df %>%
    mutate(mds_value = .data[[axis_col]],
           status    = if_else(chrom == chr &
                               mds_value <= max_thr &
                               mds_value >= min_thr,
                               "outlier", "non-outlier")) %>%
    ggplot(aes(x = midpos / 1e6, y = mds_value)) +
    geom_point(aes(colour = status), size = 1.2, alpha = 0.6) +
    scale_colour_manual(values = c("non-outlier" = "grey40", "outlier" = "red"),
                        name = "") +
...
plots_mds <- purrr::map(seq_len(nrow(info)),
                        ~ mds_plot(mds, info[.x, ]))
...
pop_info <- read.table(
  "/local_path/pmax2023/out/13.MDS_pca_het_LD/local_pca/pop_info_rm8.txt",
  header = TRUE) %>%
...
local_pca <- function(LGC) {
  ...
  cov_path <- file.path(
    "/local_path/pmax2023/out/13.MDS_pca_het_LD/local_pca",
    paste0(LGC, ".cov"))
...
het_plot <- function(LGC) {
  ...
  het_path <- file.path(
    "/local_path/pmax2023/out/13.MDS_pca_het_LD/het",
    paste0(LGC, ".het"))
...
ld_plot <- function(LGC) {
  ...
  # Load LD (all individuals)
  ld_path <- file.path(
    "/local_path/pmax2023/out/13.MDS_pca_het_LD/LD",
    paste0(LGC, ".ld")
  )
...
################################################################################
# Assemble multi-panel figure (A / (B | C | D)) and save for all inversions
################################################################################
...
save_inversion_fig <- function(LGC) {
  ...
  fig_dir  <- "/local_path/pmax2023/out/13.MDS_pca_het_LD/figures"
